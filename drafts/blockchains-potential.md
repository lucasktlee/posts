...
Now, to keep track of the _chronology_ of transactions, each block, _H<sub>b</sub>,_ contains a “pointer” to the previous block, in the form of a hash of the previous block, _H<sub>(b - 1)</sub>._ This is the “chain” in the blockchain. The transactions recorded in the blockchain are rendered effectively tamper proof thanks to one interesting property of the hash function used to generate each block: the output of a hash function is highly sensitive to changes in the input data, the slightest change to the input changes the output hash unpredictably. Hence, if you change a transaction earlier in the blockchain, you would change the hash of each subsequent block, and thus you would need to redo the proof of work done to validate each subsequent block until the current block. Hence, to make a permanent change to a transaction that occurred in the past, you would need to  
